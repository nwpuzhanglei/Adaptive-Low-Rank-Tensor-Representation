function [ rL ] = LRTDCF(Y, Mask, tau0, opts, L0)
% FUN: Low Rank Decompositionn with Clustering Factors
%
%% INPUT:
%
...
for iter = 1 : (nBurnin + nCollect)
    ...
    if opts.time
        tic
        lambda = SampleLambdaC(U, lambda, gamma, nzVal, nzIndex, tau0, Nway, K);
        %lambda = SampleLambda(nzIndex, nzVal, nzNum, tau0, lambda, U, gamma, R, K, Nway);
        t1 = toc;
        fprintf('\n lambda = %.4fs\n', t1);
    else
        lambda = SampleLambdaC(U, lambda, gamma, nzVal, nzIndex, tau0, Nway, K);
    ...
    if opts.time
        tic
        U = SampleU(nzIndex, nzVal, tau0, lambda, U, M, T, K, Nway);
        t4 = toc;
        fprintf('U = %.4fs\n', t4);
    else
        U = SampleU(nzIndex, nzVal, tau0, lambda, U, M, T, K, Nway);
...
function U = SampleU(nzIndex, nzVal, tau0, lambda, U, M, T, K, Nway)
...
for k = 1 : K
    U{k} = SampleUC(U, lambda, nzVal, nzIndex, T(k), M(k), tau0, Nway, K, k);
end
